...
// 模拟用户数据 (包含姓名)
const users = [
  { id: '1', username: 'admin', password: 'password', isAdmin: true, isDoctor: false, isPatient: false, name: '超级管理员' },
  { id: '101', username: 'doctor1', password: 'password', isAdmin: false, isDoctor: true, isPatient: false, name: '医生1' },
  { id: '102', username: 'doctor2', password: 'password', isAdmin: false, isDoctor: true, isPatient: false, name: '医生2' },
  { id: '1001', username: 'patient1', password: 'password', isAdmin: false, isDoctor: false, isPatient: true, name: '患者1' },
  { id: '1002', username: 'patient2', password: 'password', isAdmin: false, isDoctor: false, isPatient: true, name: '患者2' },
  { id: '1003', username: 'patient3', password: 'password', isAdmin: false, isDoctor: false, isPatient: true, name: '患者3' },
];

// 模拟医生数据
const doctors = users.filter(user => user.isDoctor).map(doctor => ({ id: doctor.id, username: doctor.username, name: doctor.name, password: doctor.password }));
let doctorIdCounter = doctors.length > 0 ? Math.max(...doctors.map(d => parseInt(d.id))) + 1 : 103; // 确保 ID 递增

// 模拟患者数据
const patientsData = users.filter(user => user.isPatient).map(patient => ({ id: patient.id, name: patient.name }));
...
Mock.mock('/api/users', 'post', (req) => {
  const { username, password, name } = JSON.parse(req.body); // 接收姓名
...
Mock.mock('/api/admin/doctors', 'get', () => {
  return {
    status: 200,
    data: doctors.map(doctor => ({ id: doctor.id, username: doctor.username, name: doctor.name })),
  };
});
